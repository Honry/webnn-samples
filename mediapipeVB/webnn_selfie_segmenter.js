...
import { WEIGHTS_COMMON } from './weights/weights_common.js';
import { transpose } from "./transpose.js";
import {Tensor} from './tensor.js';
/* eslint max-len: ['error', {'code': 120}] */

// Selfie-Segmenter WebNN model
export class WebnnSelfieSegmenter {
  constructor(deviceType) {
    ...
    this.weightsUrl_ = './weights/nchw';
    // Different layouts share the same bias files
    this.biasUrl_ = './weights/common';
    this.inputShape =
      this.layout === 'nhwc' ? [1, 256, 256, 3] : [1, 3, 256, 256],
    this.outputShape = [1, 256, 256, 1];
    // this.saveFile();
  }
  ...
  async buildConv_(input, index, activation = '', options = {}, isDepthwise = false) {
    const weightsName = `${this.weightsUrl_}/conv${index}_w`;
  ...
  async load() {
    ...
    let convTransposeW = await buildConstantByNpy(
      this.builder_,
      `${this.weightsUrl_}/convTranspose_w`
    );
    if (this.layout === 'nhwc') {
      const [desc, data] = await buildConstantByNpy(
        this.builder_,
