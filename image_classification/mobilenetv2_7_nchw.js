...
export class MobileNetV27Nchw {
  constructor(dataType) {
    ...
    this.weightsUrl_ = '';
    if (dataType === 'float32') {
      this.weightsUrl_ = './weights/mobilenet-fp32/';
    } else if (dataType === 'float16') {
      this.weightsUrl_ = './weights/mobilenet-fp16/';
    } else {
  ...
  async buildConv_(input, name, clip = true, options = {}) {
    const weights =
        await buildConstantByNpy(this.builder_, `${this.weightsUrl_}w${name}.npy`);
    // Only node 97 has no bias input
    if (name !== '97') {
      const bias =
          await buildConstantByNpy(this.builder_, `${this.weightsUrl_}b${name}.npy`);
      options.bias = bias;
    }
    if (clip) {
  ...
  async buildLinearBottleneck_(
    ...
    const conv1x1clip = await this.buildConv_(input, convNameArray[0]);
    const options = {
      padding: [1, 1, 1, 1],
      groups: group,
      strides: [stride, stride],
    };
    const conv3x3clip = await this.buildConv_(
        conv1x1clip, convNameArray[1], true, options);
    const conv1x1Linear = await this.buildConv_(
        conv3x3clip, convNameArray[2], false);
